...
DECLARE_OP_COVERTER(TransformLandmarksTflite);

MNN::OpType TransformLandmarksTflite::opType(int quantizedModel) {
//    DCHECK(!quantizedModel) << "Not support quantized model";
    return MNN::OpType_TransformLandmarks;
}

MNN::OpParameter TransformLandmarksTflite::type(int quantizedModel) {
    return MNN::OpParameter_TransformLandmarksParam;
}

void TransformLandmarksTflite::run(MNN::OpT *dstOp, const std::unique_ptr<tflite::OperatorT> &tfliteOp,
...
REGISTER_CONVERTER(TransformLandmarksTflite, BuiltinOperator_TransformLandmarks);
